...
The intended abundance-scale workflow is::
    ...
    QC
      ...
      -> log2 transformation
      -> optional batch correction (log2 scale only)
      -> downstream statistics

``DataFrame.attrs["abundance_scale"]`` records the current abundance scale across
these in-memory operations. Because dataframe attributes may not survive export to
disk, users loading previously transformed data may explicitly pass
``abundance_scale="log2"`` to :func:`apply_batch_correction` after confirming the
scale during QC.
"""
...
def apply_batch_correction(
    ...
    """Remove estimable batch effects with protein-wise linear models.

    Parameters
    ----------
    df
        Proteomics dataframe. Abundance columns are samples and rows are proteins.
    metadata
        Sample metadata containing batch and biological-group assignments.
    batch_col
        Metadata column containing batch labels.
    group_col
        Metadata column containing biological-group labels to preserve.
    abundance_scale
        Scale of the supplied abundance values. Pass ``"log2"`` for data
        transformed outside this module. If omitted, the function accepts the
        ``abundance_scale="log2"`` marker set by :func:`apply_log2_transformation`.

    Returns
    -------
    corrected_df : pandas.DataFrame
        New dataframe with corrected abundance columns and unchanged metadata columns.
    protein_summary : pandas.DataFrame
        Per-protein model status, observations, and estimated batch coefficients.
    summary : dict
        Method, design, confounding status, and correction counts.

    Raises
    ------
    ValueError
        If metadata are invalid, samples cannot be matched, the design is completely
        confounded, the supplied values are not confirmed as log2-scaled, or batch
        adjustment cannot be defined.

    Notes
    -----
    For each protein, the model is ``abundance ~ biological group + batch``.
    Only the centered estimated batch component is removed, retaining the observed
    protein mean and modeled biological-group effect. Missing values are never imputed.
    This assumes additive batch effects on the supplied abundance scale and a design
    in which biological group and batch can be estimated separately.
    """
    resolved_scale = abundance_scale if abundance_scale is not None else df.attrs.get(
        "abundance_scale"
    )
    if not isinstance(resolved_scale, str) or resolved_scale.strip().lower() != "log2":
        raise ValueError(
            "Batch correction requires abundances confirmed to be on a log2 scale. "
            "Use apply_log2_transformation() first, or pass abundance_scale='log2' "
            "for data whose scale has already been verified by QC."
        )

    abundance_cols = _resolve_abundance_columns(df, metadata)
    abundance_df = _copy_numeric_abundances(df, abundance_cols)
    # Metadata row order is authoritative and already matches abundance_cols.
    matched_metadata = _validate_metadata(metadata).reset_index(drop=True)
    matched_metadata.index = abundance_cols

    # Validate complete metadata before constructing the design matrix.
    for column, label in ((batch_col, "batch"), (group_col, "biological group")):
        if column not in matched_metadata.columns:
            raise ValueError(f"Metadata {label} column not found: {column}")
        if matched_metadata[column].isna().any():
            raise ValueError(f"Metadata {label} values contain missing entries.")

    batch_labels = matched_metadata[batch_col].astype(str)
    group_labels = matched_metadata[group_col].astype(str)
    if batch_labels.nunique() < 2:
        raise ValueError("Batch correction requires at least two distinct batches.")
    if group_labels.nunique() < 2:
        raise ValueError(
            "Batch correction requires at least two distinct biological groups."
        )

    contingency = pd.crosstab(batch_labels, group_labels)
    print("Batch × biological-group contingency table:")
    print(contingency)

    # Encode intercept, group, and batch while retaining one reference level each.
    design = pd.DataFrame({"intercept": np.ones(len(abundance_cols))}, index=abundance_cols)
    ...
    design = pd.concat([design, group_design, batch_design], axis=1).astype(float)

    # Rank deficiency in the full design indicates effects cannot be separated.
    design_rank = int(np.linalg.matrix_rank(design.to_numpy()))
    if design_rank < design.shape[1]:
        message = (
            "Batch correction was not performed because the model design matrix is "
            "rank deficient. This may indicate complete confounding between batch "
            "and biological group, or redundant metadata categories. The requested "
            "effects cannot be estimated separately and require design review."
        )
        print(message)
        raise ValueError(message)

    # Warn about imbalance only after complete confounding has been excluded.
    empty_cells = bool((contingency == 0).any().any())
    batch_sizes = batch_labels.value_counts()
    batch_size_ratio = float(batch_sizes.max() / batch_sizes.min())
    positive_cells = contingency.to_numpy()
    positive_cells = positive_cells[positive_cells > 0]
    cell_count_ratio = float(positive_cells.max() / positive_cells.min())
    unbalanced_design = (
        empty_cells or batch_size_ratio > 2 or cell_count_ratio > 2
    )
    if unbalanced_design:
        print(
            "Warning: the batch-by-group design is unbalanced. Batch estimates "
            "may be sensitive to the available samples even though complete "
            "confounding was not detected."
        )

    batch_columns = batch_design.columns.tolist()
    batch_positions = [design.columns.get_loc(column) for column in batch_columns]
    design_array = design.to_numpy(dtype=float)
    corrected_abundances = abundance_df.copy()
    protein_summary_rows: list[dict[str, Any]] = []

    # Prefer the known accession column while retaining row index as a fallback.
    protein_id_col = "Accession" if "Accession" in df.columns else None
    for row_position, (row_index, values) in enumerate(abundance_df.iterrows()):
        observed_mask = values.notna().to_numpy()
        observed_count = int(observed_mask.sum())
        row_summary: dict[str, Any] = {
            "protein_identifier": (
                df.iloc[row_position][protein_id_col]
            ...
            "estimated_batch_coefficients": {},
        }

        observed_design = design_array[observed_mask]
        if observed_count <= design.shape[1]:
            row_summary["reason_not_corrected"] = "insufficient observations"
            protein_summary_rows.append(row_summary)
            continue
        if np.linalg.matrix_rank(observed_design) < design.shape[1]:
            row_summary["reason_not_corrected"] = (
                "model not estimable for observed samples"
            )
            protein_summary_rows.append(row_summary)
            continue

        # Fit available observations only; missing abundance values remain missing.
        observed_values = values.to_numpy(dtype=float)[observed_mask]
        coefficients, _, _, _ = np.linalg.lstsq(
            observed_design, observed_values, rcond=None
        )
        batch_component = (
            observed_design[:, batch_positions] @ coefficients[batch_positions]
        )
        centered_batch_component = batch_component - batch_component.mean()
        corrected_values = observed_values - centered_batch_component
        observed_positions = np.flatnonzero(observed_mask)
        corrected_abundances.iloc[row_position, observed_positions] = corrected_values

        row_summary["correction_status"] = "corrected"
        row_summary["estimated_batch_coefficients"] = {
            column: float(coefficients[design.columns.get_loc(column)])
            for column in batch_columns
        }
        protein_summary_rows.append(row_summary)

    protein_summary = pd.DataFrame(protein_summary_rows)
    proteins_corrected = int(
        protein_summary["correction_status"].eq("corrected").sum()
    )
    proteins_not_corrected = int(len(protein_summary) - proteins_corrected)

    corrected_infinite_count = int(
        np.isinf(corrected_abundances.to_numpy(dtype=float)).sum()
    )
    if corrected_infinite_count:
        raise ValueError(
            "Batch correction produced infinite abundance values. Review the model "
            "inputs and design before proceeding."
        )

    corrected_df = df.copy()
    corrected_df[abundance_cols] = corrected_abundances
    corrected_df.attrs["abundance_scale"] = "log2"
    print(f"Batch column used: {batch_col}")
    print(f"Biological-group column preserved: {group_col}")
    print(f"Number of batches: {batch_labels.nunique()}")
    print(f"Proteins corrected: {proteins_corrected}")
    print(f"Proteins not corrected: {proteins_not_corrected}")
    print(
        "Protein-wise linear batch correction completed. Only the estimated batch "
        "component was removed; the modeled biological-group effect was retained. "
        "Missing values were not imputed, and the original dataframe was not modified."
    )
    print(
        "Assumption: batch effects are additive on this abundance scale, metadata are "
        "reliable, and biological group is not confounded with batch."
    )

    summary: dict[str, Any] = {
        "method": "protein_wise_linear_model",
        "model": f"abundance ~ {group_col} + {batch_col}",
        "batch_col": batch_col,
        "group_col": group_col,
        ...
        "number_of_rows": len(df),
        "sample_ids": abundance_cols,
        "samples_per_batch": batch_sizes.to_dict(),
        "samples_per_group": group_labels.value_counts().to_dict(),
        "batch_group_cell_count_ratio": cell_count_ratio,
        "design_rank": design_rank,
        "design_columns": design.columns.tolist(),
        "unbalanced_design": unbalanced_design,
        "proteins_corrected": proteins_corrected,
        "proteins_not_corrected": proteins_not_corrected,
        "confounding_status": "not_detected",
        "infinite_count": 0,
    }
    return corrected_df, protein_summary, summary
...
# )
#
# df_corrected, protein_summary, batch_info = apply_batch_correction(
#     df_log2,
#     metadata=metadata,
#     batch_col="batch",
#     group_col="group",
